...
const unsigned char windfly [] PROGMEM = {
  ...
  , 
...
//////////////////ENCODER///////////////////////
#define Enter 9 // modificar para 12 no arduino uno porque a biblioteca tone ac usa os pinos 8 e 9 no atmega328
#define ENCODER_STEP 4
const int PIN_LIGHT =  11;      // the number of the LED pin
...
MenuItem m_som = MenuItem(NULL, MENU_SOM); //som
...
void renderVario()
{
  if (true == varioState) {
    ...
    */
    ...
    display.drawRect(10, 24, 84, 48, BLACK);
    display.drawRect(0, 0, 11, 24, BLACK); 
    ...
    if (vario >= 0)
      ...
      display.fillRect(0, 24, 10, round(abs(vario) * 10), BLACK);
    else
      display.fillRect(0, 24, 10, -round(abs(vario) * 10), BLACK);
...
void renderMenu(MenuItem newMenuItem = menu.getCurrent(), uint8_t dir = 2)
{
  ...
  if (newMenuItem.getShortkey() != MENU_LEFT) {
    if (newMenuItem.getShortkey() < 10)
      ...
      display.println(F("--- OPCOES ---"));
    else if (newMenuItem.getShortkey() == MENU_RECRESET)
      display.println(F("-- MEMORIA -- "));
  }
      ...
      case MENU_QNH:
        {
          ...
          display.print(average_pressure);
          display.setTextSize(1);
      ...
      case MENU_CONTRASTE:
        {
          ...
          if (conf.contrast_default <= 0) {
            conf.contrast_default = 30;//0
          }
          else if (conf.contrast_default >= 100) {
            conf.contrast_default = 50;//100
          }
...
void menuSetup()
{
  m_vario.name = F("Tela Principal"); //Vario
  m_options.name = F("Opcoes"); //Options
  m_stats.name = F("Memoria"); //Stats
  m_retour.name = F("Retornar"); //Retour
  m_tare.name = F("Zerar Alt"); //Tare
  m_qnh.name = F("QNH"); //QNH
  m_altitude.name = F("Altitude"); //Altitude
  m_montee.name = F("Acendente"); //Montée
  m_descente.name = F("Desccendente"); //Descente
  m_light.name = F("Luzes"); //eclairage
  m_contrast.name = F("Contraste"); //eclairage
  m_date.name = F("Data"); //date
  m_som.name = F("SOM"); //som
  m_retour2.name = F("Retornar"); //Retour
  ...
  m_date.addAfter(m_som);
  m_som.addAfter(m_retour);
...
void setup()
{
  ...
  display.begin();
  display.setContrast(40);//conf.contrast_default
  display.setTextWrap(false);



  display.clearDisplay();
  display.drawBitmap(0, 0,  windfly, 84, 48, BLACK);
  display.display();
  delay(5000);
