...
#include <SoftwareSerial.h> 
  ...
  // Software Serial 
int pinRx = 2;  // Recebe dados NMEA do GPS
int pinTx = 3;  // Envia dados  NMEA para o Kobo 
...
#define UART_SPEED 57600                //define serial transmision speed (9600,19200, etc...)
#define PROTOCOL 1                       //define NMEA output: 1 = $LK8EX1, 2 = FlymasterF1, 3 = Off-Line Test with Serial Monitor (Arduino) 
...
void setup()                // setup() function to setup all necessary parameters before we go to endless loop() function
{
  ...
  Wire.begin(0x60);                   // lets init i2c protocol
...
if ((millis() >= (get_time3+(1000/NMEA_OUT_per_SEC))) && (PROTOCOL == 2))       //every NMEA_OUT/second send NMEA output over serial port (Flymaster F1 Protocol)
  ...
  +"m,3");
    ...
    Serial.print("*");                     //end of protocol string
    Serial.print(checksum_end,HEX);      //print calculated checksum on the end of the string in HEX 
    Serial.println(); 
